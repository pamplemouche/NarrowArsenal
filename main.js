...
// 1. SHADERS : On reste sur du très basique
...
// 2. JOUEUR : Positionné pour voir le cube direct
let player = { x: 0, y: 1.7, z: 5, rotY: 0, rotX: 0 };
let input = { x: 0, y: 0 };


// 3. GÉOMÉTRIE : Un sol et UN cube
let verts = [], colors = [], indices = [];
function box(x, y, z, w, h, d, r, g, b) {
    let s = verts.length / 3;
    verts.push(x-w,y,z-d, x+w,y,z-d, x+w,y+h,z-d, x-w,y+h,z-d,
...
box(0, -0.1, 0, 100, 0.1, 100, 0.2, 0.6, 0.2); // Sol vert
box(0, 0, -5, 1, 2, 1, 0.8, 0.1, 0.1);         // Cube rouge devant
...
// 4. MATHS : Matrices manuelles sans erreur
...
function draw() {
    ...
    // Mouvement
    player.z += (Math.cos(player.rotY) * input.y - Math.sin(player.rotY) * input.x) * 0.15;
    player.x += (Math.sin(player.rotY) * input.y + Math.cos(player.rotY) * input.x) * 0.15;
